...
public class score32 : MonoBehaviour {
	...
	//idxは選択中のプレイヤーを表す変数
	int idx;
	//public GameObject[] frontCircle = new GameObject[8];
	...
	void score_player(int idx/*idx : 選択中のプレイヤーを表す変数*/){
		...
		if (clplayer [idx].player_win == 0) {
			if (Input.GetKeyDown (KeyCode.O)) {
				...
				savefile ();
				//Debug.Log (clplayer [idx].cl_o_num);
			}
		}
		//誤答時はxキーを押す
		if (clplayer [idx].player_lose == 0) {
			if (Input.GetKeyDown (KeyCode.X)) {
				...
				savefile ();
				//Debug.Log (clplayer [idx].cl_x_num);
			}
		}
		//スルー時はtキーを押す
	...
	void calc_score(){
		...
		for (int i = 0; i < playerNum; i++) {
			...
			clplayer [i].cl_x_num = 7 - clplayer [i].lstClassX [countLast];
			clplayer [i].byScore = clplayer [i].cl_o_num * clplayer [i].cl_x_num;
		}
	...
	void csvload(){
		int i = 0;
		//ファイル名は2r-1.csv
		TextAsset csv = Resources.Load ("3r-2")as TextAsset;
	...
	void readfile(){
		int i = 0;
		FileInfo fi = new FileInfo (Application.dataPath + "/" + "3r-2.csv");
	...
	//Start時にのみ実行
	void init_file(){
		StreamWriter sw = new StreamWriter(Application.dataPath + "/" + "3r-2Result.csv",false);
		sw.WriteLine ();
		sw.Flush ();
		sw.Close ();
	}
	...
	void savefile(){
		...
		FileInfo fi = new FileInfo (Application.dataPath + "/" + "3r-2Result.csv");
	...
	void display_score(){
		...
		for (int i = 0; i < playerNum; i++) {
			...
			string strScoreX = "";
			string strScore = "";
			strScoreO = "scoreO" + (i + 1).ToString ();
			strScoreX = "scoreX" + (i + 1).ToString ();
			strScore = "score" + (i + 1).ToString ();
			GameObject textScoreO = GameObject.Find (strScoreO);
			GameObject textScoreX = GameObject.Find (strScoreX);
			GameObject textScore = GameObject.Find (strScore);
			textScoreO.GetComponent<TextMesh> ().text = clplayer [i].cl_o_num.ToString();
			textScoreX.GetComponent<TextMesh> ().text = clplayer [i].cl_x_num.ToString ();
			textScore.GetComponent<TextMesh> ().text = clplayer [i].byScore.ToString ();
		}
	}
