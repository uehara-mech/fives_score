...
public class score31 : MonoBehaviour {
	...
	public GameObject[,] x_display = new GameObject[8, 3];
	...
	void readfile(){
		int i = 0;
		FileInfo fi = new FileInfo (Application.dataPath + "/" + "3r-1.csv");
	...
	//Start時にのみ実行
	void init_file(){
		StreamWriter sw = new StreamWriter(Application.dataPath + "/" + "3r-1Result.csv",false);
		sw.WriteLine ();
		sw.Flush ();
		sw.Close ();
	}
	...
	void savefile(){
		...
		FileInfo fi = new FileInfo (Application.dataPath + "/" + "3r-1Result.csv");
