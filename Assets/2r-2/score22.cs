...
public class score22 : MonoBehaviour {
	...
	public GameObject[,] x_display = new GameObject[12, 2];
	...
	void readfile(int round_idx){
		int i = 0;
		FileInfo fi = new FileInfo (Application.dataPath + "/" + "2r-2_" + round_idx + ".csv");
	...
	//Start時にのみ実行
	void init_file(){
		StreamWriter sw = new StreamWriter(Application.dataPath + "/" + "2r-2Result.csv",false);
		sw.WriteLine ();
		sw.Flush ();
		sw.Close ();
	}
	...
	void savefile(){
		...
		FileInfo fi = new FileInfo (Application.dataPath + "/" + "2r-2Result.csv");
		...
		if (countLast == 0) {
			if (switch_set_idx == 0) {
				if (Input.GetKeyDown (KeyCode.Alpha1)) {
					init_player (1);
					roundName.GetComponent<TextMesh> ().text = "2R-2 1st Set";
				} else if (Input.GetKeyDown (KeyCode.Alpha2)) {
					init_player (2);
					roundName.GetComponent<TextMesh> ().text = "2R-2 2nd Set";
				} else if (Input.GetKeyDown (KeyCode.Alpha3)) {
					init_player (3);
					roundName.GetComponent<TextMesh> ().text = "2R-2 3rd Set";
				} else if (Input.GetKeyDown (KeyCode.Alpha4)) {
					init_player (4);
					roundName.GetComponent<TextMesh> ().text = "2R-2 4th Set";
				} 
			}
