...
public class scoreex2 : MonoBehaviour {

	public int playerNum = 12;
	public int player_real_num;

	//プレイヤーの各種属性を収めたクラスを12人分作る
	public playerclass[] clplayer = new playerclass[12];
	public GameObject[] x_display = new GameObject[12];
	public Material[] myMaterial = new Material[5];
	//idxは選択中のプレイヤーを表す変数
	int idx;
	public GameObject[] frontCircle = new GameObject[12];

	public GameObject[] backCircle = new GameObject[12];

	public GameObject[] selectcube = new GameObject[12];
	public GameObject qNum;

	public GameObject[] hidecube=new GameObject[12];
	...
	void init_player(){
		...
		for (int i = 0; i < playerNum; i++) {
			...
			clplayer [i].paper_class = "111";
			clplayer [i].school_class = "";
			clplayer [i].p_nameClass = "a";
			...
			string str_x = "x (" + (i + 1).ToString ()+")";
			x_display [i] = GameObject.Find (str_x);

			string hidecubename = "hideCube (" + (i + 1).ToString () + ")";
			hidecube [i] = GameObject.Find (hidecubename);
		}
		//csvファイルの読み込み
		//csvload ();
		readfile();
		...
		for (int i = 0; i < playerNum; i++) {
			if (i <= player_real_num) {
				hidecube [i].SetActive (false);
			}
		}
	...
	void readfile(){
		int i = 0;
		FileInfo fi = new FileInfo (Application.dataPath + "/" + "ex2.csv");
	...
	//Start時にのみ実行
	void init_file(){
		StreamWriter sw = new StreamWriter(Application.dataPath + "/" + "ex2Result.csv",false);
		sw.WriteLine ();
		sw.Flush ();
		sw.Close ();
	}
	...
	void savefile(){
		...
		FileInfo fi = new FileInfo (Application.dataPath + "/" + "ex2Result.csv");
	...
	void display_X(){
		for (int i = 0; i < playerNum; i++) {
			if (clplayer [i].cl_x_num == 0) {
				x_display [i].GetComponent<Renderer> ().material = myMaterial [0];
			} else if (clplayer [i].cl_x_num == 1) {
				x_display [i].GetComponent<Renderer> ().material = myMaterial [1];
			}
		}
	}
	...
	void losePlayer(){
		...
		for (int i = 0; i < playerNum; i++) {
			...
			if (clplayer [i].lstClassX [countLast] >= x_max) {
				...
				objscoreName.GetComponent<MeshRenderer> ().material = myMaterial [6];
			} else {
				if (clplayer [i].player_win == 0) {
					objscoreName.GetComponent<MeshRenderer> ().material = myMaterial [5];
